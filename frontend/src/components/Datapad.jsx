...
function Datapad() {
  ...
  useEffect(() => {
    ...
    console.log('[Datapad] Initializing for character:', characterId);
    ...
    newSocket.on('connect', () => {
      console.log('[Datapad] Socket connected:', newSocket.id);
      newSocket.emit('join_character', characterId);
      console.log('[Datapad] Joined character room:', characterId);
    });

    newSocket.on('new_message', (msg) => {
      console.log('[Datapad] New message received:', msg);
      ...
      if (!isFromMe && isForMe) {
        console.log('[Datapad] Playing notification sound');
        playNotificationSound();
      } else {
        console.log('[Datapad] Not playing sound - isFromMe:', isFromMe, 'isForMe:', isForMe);
      }
    });

    newSocket.on('disconnect', () => {
      console.log('[Datapad] Socket disconnected');
    });

    newSocket.on('connect_error', (error) => {
      console.error('[Datapad] Socket connection error:', error);
    });

    return () => {
      console.log('[Datapad] Cleaning up socket connection');
  ...
  const viewMessage = async (messageId) => {
    try {
      const token = localStorage.getItem('token');
      console.log('[Datapad] Viewing message:', messageId, 'as character:', characterId);
      ...
      console.log('[Datapad] Thread participants:', response.data.thread.map(m => ({
        id: m.id,
  ...
  if (!character) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-white">Loading datapad...</div>
      </div>
    );
  }
        ...
        <div className="divide-y divide-green-800">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => {
              ...
              console.log('[Datapad] Message display:', {
                msg_id: msg.id,
                subject: msg.subject,
                isUnread,
                iParticipated,
